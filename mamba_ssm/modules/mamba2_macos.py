...
class Mamba2MacOS(nn.Module):
    ...
    def __init__(
        ...
        self.A_log = nn.Parameter(
            torch.log(
                torch.rand(self.d_inner, d_state, **factory_kwargs)
                * (A_init_range[1] - A_init_range[0])
                + A_init_range[0]
            )
        )
        ...
        if activation == "silu":
            ...
            raise ValueError(
                f"Unsupported activation: {activation}. Supported: 'silu', 'swish', 'gelu'"
            )
    ...
    def forward(
        self, hidden_states, inference_params=None, seq_idx=None, cu_seqlens=None
    ):
        """
        ...
        dt, B, C = torch.split(
            x_proj, [self.nheads, self.d_state, self.d_state], dim=-1
        )

        # Reshape back to proper dimensions
        dt = rearrange(
            dt, "(b l) h -> b h l", l=seq_len
        )  # (batch_size, nheads, seq_len)
        dt = repeat(
            dt, "b h l -> b (h p) l", p=self.headdim
        )  # (batch_size, d_inner, seq_len)
        dt = F.softplus(
            dt + repeat(self.dt_bias, "h -> (h p)", p=self.headdim)[..., None]
        )

        B = rearrange(
            B, "(b l) n -> b 1 n l", l=seq_len
        )  # (batch_size, 1, d_state, seq_len)
        C = rearrange(
            C, "(b l) n -> b 1 n l", l=seq_len
        )  # (batch_size, 1, d_state, seq_len)

        # SSM matrices per head
        A = -torch.exp(self.A_log).view(
    ...
    def step(self, hidden_states, conv_state, ssm_state):
        ...
        dA = reduce(
            dA, "b h p n -> b h n", "mean"
        )  # Average over headdim to get (batch, nheads, d_state)
        ...
        y_ssm = repeat(
            y_ssm, "b h -> b h p", p=self.headdim
    ...
    def allocate_inference_cache(self, batch_size, max_seqlen, dtype=None):
        ...
        ssm_state = torch.zeros(
            batch_size, self.nheads, self.d_state, device=device, dtype=ssm_dtype
        )
