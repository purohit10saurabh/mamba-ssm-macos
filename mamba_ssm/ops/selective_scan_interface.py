        ...
        if not has_cuda_support:
            ...
            if return_last_state:
                ...
                ctx.save_for_backward(
                    u, delta, A, B, C, D, None if z is None else z, delta_bias
                )
...
class MambaInnerFn(torch.autograd.Function):
    ...
    def forward(
        ...
        if causal_conv1d_cuda is None:
            raise ImportError(
                "causal_conv1d_cuda is not available. Please install causal-conv1d or use the reference implementation."
            )
