...
@triton.autotune(
    configs=[
        ...
        triton.Config(
            {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 64}, num_stages=3, num_warps=4
        ),
        triton.Config(
            {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 128}, num_stages=3, num_warps=4
        ),
    ],
    key=["chunk_size", "hdim"],
)
...
def _chunk_scan_fwd_wip(
    cb, x, dt, dA_cumsum, C, B, states, D=None, z=None, seq_idx=None
):
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    _, _, ngroups, dstate = C.shape
...
def _chunk_scan_bwd_dC(prev_states, dA_cumsum, dout, seq_idx=None, C=None, ngroups=1):
    ...
    dC = torch.empty(
        batch, seqlen, nsplits, ngroups, dstate, device=dout.device, dtype=torch.float32
    )
    grid_dc = lambda META: (
        triton.cdiv(chunk_size, META["BLOCK_SIZE_M"])
        * triton.cdiv(dstate, META["BLOCK_SIZE_N"]),
...
def _chunk_scan_bwd_ddAcs_unstable(x, dt, out, dout, ddt, D=None, subtract_ddtdt=True):
    ...
    if D is not None:
        BLOCK_SIZE_actual = _chunk_scan_bwd_ddAcs_unstable_kernel.best_config.kwargs[
            "BLOCK_SIZE_M"
        ]
        n_valid_blocks = (chunk_size + BLOCK_SIZE_actual - 1) // BLOCK_SIZE_actual
        dD = dD[:n_valid_blocks].sum(dim=(0, 1, 2)).to(dtype=D.dtype)
...
def _chunk_scan_bwd_ddAcs_stable_old(x, dt, dA_cumsum, dout, cb):
    ...
    BLOCK_SIZE_M_actual = _chunk_scan_bwd_ddAcs_stable_kernel_old.best_config.kwargs[
        "BLOCK_SIZE_M"
    ]
    n_valid_blocks = (chunk_size + BLOCK_SIZE_M_actual - 1) // BLOCK_SIZE_M_actual
    ddA_cumsum = ddA_cumsum[:, :, :, :n_valid_blocks].sum(dim=3)
...
class ChunkScanFn(torch.autograd.Function):
    ...
    def forward(ctx, B, C, x, dt, dA_cumsum, prev_states, D=None, z=None):
        ...
        ctx.save_for_backward(
            out if z is None else out_x, B, C, CB, x, dt, dA_cumsum, prev_states, D, z
        )
        return out
    ...
    def backward(ctx, dout):
        ...
        dprev_states = _chunk_scan_bwd_dstates(
            C, dA_cumsum, dout, dtype=prev_states.dtype
        )
