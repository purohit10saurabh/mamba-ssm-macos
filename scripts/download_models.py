...
def download_model(model_type, model_dir):
    ...
    if model_type == "mamba1":
        ...
        config_path = hf_hub_download("state-spaces/mamba-130m", "config.json", local_dir=local_dir, resume_download=True)
        print("📦 Downloading pytorch_model.bin...")
        model_path = hf_hub_download("state-spaces/mamba-130m", "pytorch_model.bin", local_dir=local_dir, resume_download=True)
        ...
        downloaded_files = [f"{model_type}-130m-config.json", f"{model_type}-130m-model.bin"]
    else:
        print("📦 Downloading all files...")
        snapshot_download("state-spaces/mamba2-130m", local_dir=local_dir, resume_download=True, local_dir_use_symlinks=False)
