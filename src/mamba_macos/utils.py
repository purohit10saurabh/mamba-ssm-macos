...
def create_fallback_model(config, device):
    ...
    class SimpleMambaLM(nn.Module):
        def __init__(self, config, device=None, dtype=None):
            ...
            self.backbone = MixerModel(d_model=config.d_model, n_layer=config.n_layer,
                                     d_intermediate=getattr(config, 'd_intermediate', 0),
                                     vocab_size=vocab_size, ssm_cfg=getattr(config, 'ssm_cfg', {}),
                                     device=device, dtype=dtype, fused_add_norm=False)
